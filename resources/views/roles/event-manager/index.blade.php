...
<div class="container mx-auto p-6">
    <h1 class="text-3xl font-semibold text-center mb-6">Event Manager List</h1>
    ...
    <div class="flex items-center justify-between mb-4">
        <!-- Search Form -->
        <form action="{{ route('role.management.event.manager.index') }}" method="GET" class="flex items-center">
            <input
                type="text"
                name="search"
        ...
        <!-- Add Event Button -->
        <a href="{{ route('role.management.event.manager.create') }}"
            ...
            <tbody class="text-gray-700 dark:text-gray-300">
                @foreach($managers as $manager)
                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition duration-150 ease-in-out">
                        <td class="py-4 px-6">{{ $manager->name }}</td>
                        <td class="py-4 px-6">{{ $manager->email }}</td>
                        <td class="py-4 px-6 text-center flex justify-center space-x-4">
                            <!-- Delete Button -->
                            <form action="{{ route('role.management.event.manager.destroy', $manager->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
