...
<div class="container mx-auto p-6">
    <h1 class="text-3xl font-semibold text-center mb-6">Photographer List</h1>
    ...
    <div class="flex items-center justify-between mb-4">
        <!-- Search Form -->
        <form action="{{ route('role.management.photographer.index') }}" method="GET" class="flex items-center">
            <input
                type="text"
                name="search"
        ...
        <!-- Add Event Button -->
        <a href="{{ route('role.management.photographer.create') }}"
            ...
            <tbody class="text-gray-700 dark:text-gray-300">
                @foreach($photographers as $photographer)
                    <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition duration-150 ease-in-out">
                        <td class="py-4 px-6">{{ $photographer->name }}</td>
                        <td class="py-4 px-6">{{ $photographer->email }}</td>
                        <td class="py-4 px-6 text-center flex justify-center space-x-4">
                            <!-- Delete Button -->
                            <form action="{{ route('role.management.photographer.destroy', $photographer->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="{{ $photographer->status === 'active' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600' }} text-white text-sm px-4 py-2 rounded-lg shadow transition"
                                        onclick="return confirm('Are you sure you want to {{ $photographer->status === 'active' ? 'deactivate' : 'activate' }} this photographer?')">
                                    {{ $photographer->status === 'active' ? 'Deactivate' : 'Activate' }}
                                </button>
                            </form>
                        </td>
                    </tr>
